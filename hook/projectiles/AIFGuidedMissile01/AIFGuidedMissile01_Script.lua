...
AIFGuidedMissile = Class(AGuidedMissileProjectile) {
    ...
    SplitThread = function(self)
		...
		local t_units = self.myAIBrain:GetUnitsAroundPoint( categories.LAND + categories.STRUCTURE, self:GetTrackingTarget():GetPosition(), self:GetBlueprint().General.TargetRadius, 'Enemy' )
