...
local WeatherDefinition = import('/Mods/FARebalanced/hook/lua/weatherdefinitions.lua')
...
function GetWeatherMarkerData(MapScale)
    ...
    if markers then
        #LOG('*INFO: We have markers')
        for k, v in markers do
          local pick = util.GetRandomFloat( 0, 20 )
          if pick > 18 then
          #LOG('*INFO: ' .. v.MapStyle)
            ...
            elseif v.type == 'Weather Definition' then
        				...
        				WeatherDefinition = {
        					MapStyle = v.MapStyle or "Evergreen",
        					WeatherTypes = {
        						{
        							Type = v.WeatherType01 or "Evergreen",
        							Chance = v.WeatherType01Chance or 0.25,
        						},
        						{
        							Type = v.WeatherType02 or "Evergreen",
        							Chance = v.WeatherType02Chance or 0.25,
        						},
        						{
        							Type = v.WeatherType03 or "Evergreen",
        							Chance = v.WeatherType03Chance or 0.25,
        						},
        						{
        							Type = v.WeatherType04 or "Evergreen",
