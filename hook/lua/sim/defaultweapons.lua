...
CarpetBombingWeapon = Class(DefaultProjectileWeapon) {
	AlternateCarpetAlgorithm = function(self,muzzle) 

		local ux, uy, uz = self.unit:GetVelocity()
		
		local vel = math.sqrt(ux*ux + uy*uy + uz*uz) * 10
		if(vel < 15.5) then return nil end 
		
		
		local proj = DefaultProjectileWeapon.CreateProjectileAtMuzzle(self, muzzle)
		local myBP = self:GetBlueprint()
		local ProjectileBP = myBP.ProjectileId
		local numProjectiles = myBP.CarpetBombNumWidthProjectiles or 3
		
		local angle = (myBP.CarpetAngleSpread or 15)
		local dd = proj.DamageData;
		
		local vx, vy, vz = proj:GetVelocity()
		
		
		local alpha = angle/360 *2 * math.pi
		
		local stepAlpha = (((angle*2) / (numProjectiles-1)) *2 * math.pi )/ 360
		local currentAlpha = -alpha
		local cproj = proj:CreateProjectile(ProjectileBP)
		
		cproj:SetVelocity(ux,uy,uz);
		cproj:SetVelocity(vel);
		cproj:PassDamageData(dd)   
		vx = ux 
		vy = uy
		vz = uz
		local iProj
		for i = 0, (numProjectiles -1) do
				currentAlpha = -alpha + i*stepAlpha
				#LOG("alpha is " .. (alpha / (math.pi *2) * 360))
				#LOG("our current alpha is " .. (currentAlpha / (math.pi *2) * 360))
				iProj = cproj:CreateProjectile(ProjectileBP)
				iProj:SetVelocity(math.cos(currentAlpha) * vx - math.sin(currentAlpha)*vz +2*ux, vy, math.sin(currentAlpha) * vx + math.cos(currentAlpha)*vz +2*uz)
				iProj:SetVelocity(vel);
				iProj:PassDamageData(dd)  
   ...
   CreateProjectileAtMuzzle = function(self, muzzle) 
		...
		local ProjectileBP = self:GetBlueprint().ProjectileId
		...
		for i = 0, (numProjectiles -1) do
			...
			if i == 0 then
				iProj = cproj:CreateProjectile(ProjectileBP)
				iProj:SetVelocity(math.cos(alpha) * vx - math.sin(alpha)*vz +2*ux, vy, math.sin(alpha) * vx + math.cos(alpha)*vz +2*uz)
				iProj:SetVelocity(vel);
				iProj:PassDamageData(dd)  
			else
				iProj = cproj:CreateProjectile(ProjectileBP)
		...
		proj:Destroy()]]--
		self.AlternateCarpetAlgorithm(self,muzzle)
    end,
}